...
if __name__ == '__main__':
    ...
    parser.add_argument('--image_size', default=84, type=int, choices=[84, 112, 224], help='input image size: 84 image-FSL, 112 video (VideoMAE-S), 224 cub')
    ...
    # K400-pretrained backbone init (required for video CD-FSAR; empty = from scratch).
    # Without this, VideoMAE-S would train from random init on the small target
    # base split, which is NOT the intended (fair) setting.
    parser.add_argument('--pretrain_path', default='', type=str,
                        help='path to K400-pretrained backbone (e.g. model/112112vit-s-140epoch.pt)')
    ...
    # Load K400-pretrained backbone (frozen transformer blocks + trainable TAA),
    # mirroring meta_train.py so good_embed shares the SAME frozen-backbone+TAA
    # feature extractor as TAMT / protonet (fair, controlled comparison).
    if params.pretrain_path:
        model = load_model(model, params.pretrain_path)
        print(f'[pretrain] loaded K400 backbone from {params.pretrain_path}')
